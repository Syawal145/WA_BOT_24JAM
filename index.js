...
const start = async (client = new Client()) => {
    ...
    await client.sendText(chat.id, `Sorry, bot tidak bisa join di grub untuk menginvite bot silahkan hubungi owner bot https://wa.me/6283849021375 max group: ${groupLimit}`).then(() => {
    ...
    // kondisi ketika batas member group belum tercapai, ubah di file settings/setting.json
        ...
        await client.simulateTyping(chat.id, true).then(async () => {
          await client.sendText(chat.id, `Hai minna~, Im SYAWAL BOT. To find out the commands on this bot type #menu`)
        })
        }
    }
    })
